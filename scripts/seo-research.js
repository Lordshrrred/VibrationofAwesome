...
function buildSeedResearch(niche, date) {
  ...
  for (const intent of SEARCH_INTENTS) {
    grouped[intent] = (niche.keywordResearch[intent] || []).map((keyword) => ({
      ...
      opportunity: "Unvalidated topic hypothesis from niche strategy; no measured volume or difficulty.",
      h2_outline: [
  ...
  const message = await client.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        SEARCH_INTENTS.join(", "),
        "",
        "For each intent, return 4 specific reader-question hypotheses. You have no live search evidence: never claim verified demand or low competition.",
        "Each candidate must have these fields:",
        "  keyword         (string)",
        "  search_intent   (one of the exact intent keys)",
        "  suggested_title (string)",
        "  opportunity     (string describing reader intent and what still needs validation)",
