...
function buildHtml(lane, title, dateStr, bodyHtml, slug, metaDescription, heroImageUrl) {
  ...
  if (!isMatt && heroImageUrl) {
    H.push("    .post-header { position:relative; overflow:hidden; padding:12rem 0 5rem; min-height:72vh; border-bottom:1px solid var(--border); background: linear-gradient(to bottom, rgba(2,10,8,0.55) 0%, rgba(2,10,8,0.82) 62%, #020a0a 100%), url('" + heroImageUrl + "') center/cover no-repeat; }");
  ...
  if (!isMatt) {
    H.push("    @media (max-width: 600px) { body { font-size: 16px; } .post-header { padding: 7rem 0 2.5rem; min-height: 50vh; } }");
  } else {
