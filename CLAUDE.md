...
- **Flagship reader-facing content** ~ `generate-post.js` runs **Opus 5** (`claude-opus-5`), migrated from Opus 4.8 on 2026-08-06. Do not downgrade without asking first. Three things about this call site are load-bearing and must move together if it is ever touched again:
  1. **`max_tokens: 16000`, not 4096.** Opus 5 thinks by default and `max_tokens` caps thinking *plus* response text together. The old 4096 was sized around the post body alone and would truncate posts mid-sentence.
  2. **`extractText(message)`, never `message.content[0].text`.** With thinking on, `content[0]` is a `thinking` block ~ reading `.text` off it returns undefined and the post comes out silently empty. `extractText()` filters to text blocks, concatenates them, throws on empty, and warns on `stop_reason: "max_tokens"`. **The same latent bug exists in ~10 other call sites** (`generate-captions.js`, `generate-pinterest-image.js`, `generate-from-inspiration.js`, `seo-research.js`, `auto-heal.js`, `lib/visual-intelligence.js`, `generate-instagram-visual.js`) ~ they are safe only because they are still on Haiku/Sonnet with thinking off. **Any of those moved to a thinking-on model must switch to a text-block filter in the same commit.**
  3. **`output_config: { effort: "medium" }`.** Opus 5's default effort is `high`. Measured on this exact prompt (steady-state, cache-read): `low` $0.071/1359 words, `medium` $0.089/1747, `high` $0.127/1977, against Opus 4.8's $0.058/~2000 words. Per-token pricing is identical between 4.8 and 5 ($5/$25 per MTok) ~ the increase is entirely thinking tokens billed as output. `medium` keeps generation near cost-neutral and tracks the ~1490-word median published post. Effort is the cost lever here; do not try to shorten posts by lowering `max_tokens` (that truncates instead).
