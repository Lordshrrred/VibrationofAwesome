...
- An assigned open GitHub issue is the guaranteed durable alert and works even
  while Matt's Mac is asleep. Gmail is a secondary channel using the existing
  OAuth client/refresh-token secrets with `gmail.send` scope; it activates once
  that OAuth grant is refreshed. Never replace either with local macOS `mail`,
  which can exit zero while no mail service is running.
...
**Favicon (fixed, stale note removed 2026-07-28):** a full favicon set now exists (`static/favicon.ico`, `favicon-{16,32,192,512}x{16,32,192,512}.png`, `apple-touch-icon.png`, `site.webmanifest`, wired in `layouts/_default/baseof.html`) ~ a clean teal star/crest mark, legible at small sizes. No further work needed here.

## Ebook cover assets, social preview images, and AI-written first-person claims (2026-07-28)

**Ebook cover artwork is fixed.** `static/images/field-guide-cover.png` (Field Guide) and `static/downloads/cover.png` (AI Engine / Creative Exoskeleton) use the Matt EarthStar cover artwork. Existing in-page references keep the canonical filenames, and the Field Guide, thank-you, user-manual, and AI Engine social-preview metadata points to the corrected covers. The obsolete hashed Field Guide and User Manual PDFs were deleted; legacy ebook delivery now routes to the current `static/downloads/voa-field-guide.pdf` and `static/downloads/voa-ai.pdf` files.

**Social preview default was an ugly transparent thumbnail (fixed 2026-07-28):** `layouts/_default/baseof.html`'s default `$socialImage` (used for the homepage and any page without a specific `image`/`social_image` param) was `static/images/StarLogo.png` ~ a thin wireframe mark on a fully transparent background, which renders as a sparse, oddly-cropped thumbnail in iMessage/Facebook/Slack previews. Default (and every static page that also hardcoded `StarLogo.png` as its `og:image`/`twitter:image` ~ 30 pages total: `aura`, `contact`, `tools`, `privacy`, `terms`, `blog`, `hubs`, `data-deletion`, plus their subpages) now points at `static/images/eartstarart.jpg`, a full-bleed opaque branded image already used successfully as the social image on `art-store`/`earthstar` pages. `field-guide`, `field-guide/thank-you`, `user-manual`, and `ai-engine` now use their corrected Matt EarthStar ebook covers for `og:image`/`twitter:image`. `StarLogo.png` is still correct and unchanged as the schema.org Organization `logo` ~ that's a different, legitimate usage (structured data icon, not a social-share thumbnail).

**AI-written first-person fabrication (audited and fixed 2026-07-28):** Boom Frequency posts (written by the "Matty BoomBoom" AI persona) had accumulated fabricated specific personal claims ~ "I threw a 40-page PDF into NotebookLM," "I built a JavaScript widget," "I spent six months building automations," "the system I built after years of crashing and burning," etc. 15 published posts were rewritten to keep the same information and voice while removing the fabricated lived-experience framing (general guidance instead of invented personal testimony). Real, verifiable claims about the site's own actual products (e.g. "I built the AI Engine guide," referring to the real `/ai-engine/` free guide) were deliberately left alone ~ those aren't fabrications. `BOOMBOT_SYSTEM` in `generate-post.js` and the system prompt in `generate-from-inspiration.js` already had a `TRUTHFULNESS RULES` block but it only banned a narrow phrase list; both are now expanded to ban the full pattern class (disguised origin-story and timeline fabrications included, not just the literal banned phrases) and to state the allowed/not-allowed framing explicitly: first person is fine for viewpoint/philosophy, never for specific unsourced lived experience. When touching either generator's system prompt again, keep this block in sync between the two files.

**Mobile: the announcement bar overlapped the site header (fixed 2026-07-28):** `static/js/announcement-bar.js`'s `#voa-announce` bar is `position: fixed; top: 0`, mobile-only (`@media(min-width:769px){display:none}`). Being fixed, it doesn't reserve space in normal document flow ~ it was sliding down on top of the site header/logo instead of pushing content down. Fixed by measuring the bar's actual rendered height in JS and applying it as `body` `padding-top` (added/removed on show/dismiss, recalculated on resize), rather than hardcoding a height that could drift if the bar's copy or font size changes.
...
- **Flagship reader-facing content** ~ `generate-post.js` runs **Opus 4.8** (`claude-opus-4-8`, `max_tokens: 4096`). Do not downgrade without asking first.

  **Do not "upgrade" this to Opus 5 either.** It was tried and deliberately reverted on 2026-08-06. Opus 5 is the same $5/$25 per MTok, so the swap looks free, but it **thinks by default and thinking tokens bill as output**. Measured on this exact prompt (steady-state, cache-read): Opus 4.8 **$0.058/post**, vs Opus 5 at `low` $0.071, `medium` $0.089, `high` (the default) $0.127. Even the cheapest Opus 5 setting costs more, so the current setup is the cost-optimal one. If it is ever revisited, all three of these must change together or it breaks:
  1. `max_tokens` must rise well above 4096 ~ on Opus 5 it caps thinking *plus* response text together, so 4096 truncates posts mid-sentence.
  2. The response reader must not use `message.content[0].text` ~ with thinking on, `content[0]` is a `thinking` block, so that read returns undefined and the post comes out **silently empty**. Use `extractText()` (already in this file).
  3. `output_config.effort` must be set explicitly; the default is `high`, the most expensive setting.

  **`extractText()` is kept even on Opus 4.8.** It is behaviour-identical here (block 0 is the text block on a thinking-off model) but costs nothing and removes the silent-empty-post failure mode. **~10 other call sites still read `content[0].text`** (`generate-captions.js`, `generate-pinterest-image.js`, `generate-from-inspiration.js`, `seo-research.js`, `auto-heal.js`, `lib/visual-intelligence.js`, `generate-instagram-visual.js`) and are safe **only** because they run Haiku/Sonnet with thinking off. Moving any of them to a thinking-on model requires switching to a text-block filter in the same commit.
