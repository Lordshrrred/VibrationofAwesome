...
**Ebook covers still say "Matt Dunn" ~ no replacement exists in the repo, do not treat this as fixed.** Both `static/images/field-guide-cover.png` (Field Guide) and `static/downloads/cover.png` (AI Engine / Creative Exoskeleton) have "MATT DUNN" baked into the image pixels (confirmed by visually opening the files ~ text search for "Matt Dunn" across the repo returns zero hits because it's only in the artwork, never in HTML/JSON). No "Matt EarthStar" version of either cover exists anywhere in the repo, and no image-generation tooling exists for ebook covers specifically (only `scripts/refresh-social-images.js`, which explicitly excludes both covers from its own logic). Per standing brand decision, the public author/cover name is Matt EarthStar; alt text everywhere already correctly says "Matt EarthStar" ~ only the artwork itself is wrong. **TODO (manual, needs real design work, not scriptable):** regenerate both covers with "Matt EarthStar" in place of "Matt Dunn" at `static/images/field-guide-cover.png` and `static/downloads/cover.png` (same filenames, same aspect ratios, so no code changes are needed once they're ready ~ just drop in the replacements). The underlying PDFs (`static/downloads/voa-field-guide.pdf`, `voa-ai.pdf`, `AI_Advantage_*.pdf`) likely have the same cover baked into their first page and were not opened/audited this pass.

**Social preview default was an ugly transparent thumbnail (fixed 2026-07-28):** `layouts/_default/baseof.html`'s default `$socialImage` (used for the homepage and any page without a specific `image`/`social_image` param) was `static/images/StarLogo.png` ~ a thin wireframe mark on a fully transparent background, which renders as a sparse, oddly-cropped thumbnail in iMessage/Facebook/Slack previews. Default (and every static page that also hardcoded `StarLogo.png` as its `og:image`/`twitter:image` ~ 30 pages total: `aura`, `contact`, `tools`, `privacy`, `terms`, `blog`, `hubs`, `data-deletion`, plus their subpages) now points at `static/images/eartstarart.jpg`, a full-bleed opaque branded image already used successfully as the social image on `art-store`/`earthstar` pages. `field-guide`, `field-guide/thank-you`, `user-manual`, and `ai-engine` previously used the Matt-Dunn-labeled covers for `og:image`/`twitter:image` ~ switched to `eartstarart2.jpg`/`eartstarart3.jpg` respectively (metadata only; the in-page `<img>` displays of the actual covers are untouched pending the manual cover replacement above, since there's no Matt-EarthStar asset to swap in). `StarLogo.png` is still correct and unchanged as the schema.org Organization `logo` ~ that's a different, legitimate usage (structured data icon, not a social-share thumbnail).
